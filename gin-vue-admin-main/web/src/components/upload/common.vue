...
<script setup>
  ...
  const checkFile = (file) => {
    ...
    const isLt5M = file.size / 1024 / 1024 < 50 // 5MB, @todo 应支持项目中设置
    ...
    if (!isVideo && !isImage) {
      ElMessage.error(
        '上传图片只能是 jpg,png,svg,webp 格式, 上传视频只能是 mp4,webm 格式,上传的音乐只能是ogg,flac格式!'
      )
    ...
    if (!isLt500K && isImage) {
      ElMessage.error('未压缩的上传图片大小不能超过 5MB，请使用压缩上传')
      fullscreenLoading.value = false
      pass = false
    }
